...
from .predictor import PredictorCNN
class AudioUtil:
    @staticmethod
    ...
    def segment_audio(audio_path):
        ...
        AudioUtil.clean_audio(test_path, output_path)
        # y is time series - values of waveform at indices indicating the sample number (time * sample rate)
        y, sr = librosa.load(output_path)
    ...
    @staticmethod
    def save_spectrogram(y_slice, title):
        D = librosa.stft(y=y_slice)
    ...
    def get_notes(audio_path):
        bounds, y_smooth, sr = AudioUtil.segment_audio(audio_path)
        ...
        for i in range(len(bounds) - 1):
            ...
            spec_save_path = f".\\data\\app\\{i}.png"
            AudioUtil.save_spectrogram(y_smooth[lower_bound:upper_bound], spec_save_path)\
            
            predictions.append(cnn.predict(spec_save_path))
    ...
    def music_notator(input_path, output_path):
        chromas, _ = AudioUtil.get_notes(input_path)
